...
<html dir="<?php echo e($site_direction); ?>" lang="<?php echo e(str_replace('_', '-', app()->getLocale())); ?>"
    class="<?php echo e($site_direction === 'rtl' ? 'active' : ''); ?>">
...
<body class="footer-offset">
                            ...
                            <div class="col-12">
                                <div class="text-center">
                                    <h2 class="color-8a8a8a">
                                        ...
                                        <?php echo e(translate('messages.You_have_new_order_Check_Please.')); ?>

                                    </h2>
                                    <hr>
                                    <button
                                        class="btn btn-primary check-order"><?php echo e(translate('messages.Ok_let_me_check')); ?></button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <!-- JS Front -->
    <script
        src="<?php echo e(dynamicAsset('public/assets/admin/vendor/hs-navbar-vertical-aside/hs-navbar-vertical-aside-mini-cache.js')); ?>">
    </script>
    ...
    <script>
        "use strict";
        var country_id = "<?php echo e(auth('admin')->user()->country_id); ?>";
        if (country_id) {
            $.ajax({
                url: '/zones/' + country_id,
                type: 'GET',
                dataType: 'json',
                success: function(data) {
                    ...
                    $.each(data, function(key, value) {
                        $('select[name=zone_id]').append('<option value="' + value.id + '">' + value
                            .name + '</option>');
                    });
                }
            });
        ...
        function route_alert(route, message, title = "<?php echo e(translate('messages.are_you_sure')); ?>", processing = false) {
            if (processing) {
                Swal.fire({
                    ...
                    input: 'text',
                    html: message + '<br/>' +
                        '<label><?php echo e(translate('messages.Enter_Processing_time_in_minutes')); ?></label>',
...
<?php /**PATH C:\wamp64\www\Mashe-Backend-main\resources\views/layouts/admin/app.blade.php ENDPATH**/ ?>