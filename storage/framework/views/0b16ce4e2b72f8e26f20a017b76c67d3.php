<?php $__env->startSection('title',translate('market_List')); ?>
...
<?php $__env->startSection('content'); ?>
    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <h1 class="page-header-title"><i class="tio-filter-list"></i> <?php echo e(translate('messages.markets')); ?> <span class="badge badge-soft-dark ml-2" id="itemCount"><?php echo e($markets->total()); ?></span></h1>
        </div>
        <!-- End Page Header -->
        ...
        <div class="card shadow--card p-0 mb-4">
            <div class="card-body">
                ...
                <div class="col-sm-6 col-md-3">
                    <div class="select-item">
                        <!-- Veg/NonVeg filter -->
                        <select name="market_model"
                                data-url="<?php echo e(url()->full()); ?>" data-filter="market_model"
                        data-placeholder="<?php echo e(translate('messages.all')); ?>" class="form-control js-select2-custom set-filter">
        ...
        <div class="row g-3 mb-3">
            <div class="col-xl-3 col-sm-6">
                <div class="resturant-card bg--1">
                    <h4 class="title" id="itemCount" ><?php echo e($markets->total()); ?></h4>
                    <span class="subtitle"><?php echo e(translate('messages.total_markets')); ?></span>
            ...
            <div class="col-xl-3 col-sm-6">
                ...
                <div class="resturant-card bg--2">
                    <?php ($active_markets = \App\Models\Market::where(['status'=>1])
                    ...
                    ->type($type)->MarketModel($typ)
                    ->count()); ?>
                    <?php ($active_markets = isset($active_markets) ? $active_markets : 0); ?>
                    <h4 class="title"><?php echo e($active_markets); ?></h4>
                    <span class="subtitle"><?php echo e(translate('messages.active_markets')); ?></span>
            ...
            <div class="col-xl-3 col-sm-6">
                <div class="resturant-card bg--3">
                    <?php ($inactive_markets = \App\Models\Market::where(['status'=>0])
                     ->whereHas('vendor', function($q){
                            $q->where('status',1);
                        })
                    ->when( isset($zone) && ($zone->id), function ($query) use ($zone) {
                    ...
                    ->type($type)->MarketModel($typ)
                    ->count()); ?>
                    <?php ($inactive_markets = isset($inactive_markets) ? $inactive_markets : 0); ?>
                    <h4 class="title"><?php echo e($inactive_markets); ?></h4>
                    <span class="subtitle"><?php echo e(translate('messages.inactive_markets')); ?></span>
            ...
            <div class="col-xl-3 col-sm-6">
                <div class="resturant-card bg--4">
                    <?php ($data = \App\Models\Market::where('created_at', '>=', now()->subDays(30)->toDateTimeString())
                    ->whereHas('vendor', function($q){
                            $q->where('status',1);
                        })
                    ->when( isset($zone) && ($zone->id), function ($query) use ($zone) {
                    ...
                    <span class="subtitle"><?php echo e(translate('messages.newly_joined_markets')); ?></span>
        ...
        <ul class="transaction--information text-uppercase">
            ...
            <li class="text--danger">
                <i class="tio-atm"></i>
                <div>
                    <?php ($market_withdraws = \App\Models\WithdrawRequest::where(['approved'=>1])->sum('amount')); ?>
                    <?php ($market_withdraws = isset($market_withdraws) ? $market_withdraws : 0); ?>
                    <span><?php echo e(translate('messages.total_market_withdraws')); ?></span> <strong><?php echo e(\App\CentralLogics\Helpers::format_currency($market_withdraws)); ?></strong>
                </div>
            </li>
        </ul>
        <!-- Transaction Information -->
                ...
                <div class="card">
                    ...
                    <div class="card-header py-2 border-0">
                        <div class="search--button-wrapper">
                            <h3 class="card-title"><?php echo e(translate('messages.markets_list')); ?></h3>
                            <form class="my-2 ml-auto mr-sm-2 mr-xl-4 ml-sm-auto flex-grow-1 flex-grow-sm-0">

                                <div class="input--group input-group input-group-merge input-group-flush">
                                    <input id="datatableSearch_" type="search" name="search" class="form-control"
                                          value="<?php echo e(request()?->search  ?? null); ?>"  placeholder="<?php echo e(translate('Ex:_search_by_market_name_of_Phone_number')); ?>" aria-label="<?php echo e(translate('messages.search')); ?>" required>
                            ...
                            <div class="hs-unfold ml-3">
                                ...
                                <div id="usersExportDropdown"
                                    ...
                                    <a target="__blank" id="export-excel" class="dropdown-item" href="<?php echo e(route('admin.market.markets-export', ['type'=>'excel',
                   request()->getQueryString()])); ?>">
                                    ...
                                    <a id="export-excel" class="dropdown-item" href="<?php echo e(route('admin.market.markets-export', ['type'=>'csv',request()->getQueryString()])); ?>">
                                        <img class="avatar avatar-xss avatar-4by3 mr-2"
                    ...
                    <div class="table-responsive datatable-custom resturant-list-table">
                        <table id="columnSearchDatatable"
                            ...
                            <tr>
                                ...
                                <th class="initial-58"><?php echo e(translate('messages.market_info')); ?></th>
                            ...
                            <tbody id="set-rows">
                            <?php $__currentLoopData = $markets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key=>$dm): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr>
                                    <td><?php echo e($key+$markets->firstItem()); ?></td>
                                    <td>
                                        <a href="<?php echo e(route('admin.market.view', $dm->id)); ?>" alt="view Market" class="table-rest-info">
                                        <img class="onerror-image" data-onerror-image="<?php echo e(dynamicAsset('public/assets/admin/img/100x100/food-default-image.png')); ?>"
                                             ...
                                             src="<?php echo e(\App\CentralLogics\Helpers::onerror_image_helper(
                                                $dm['logo'] ?? '',
                                                dynamicStorage('storage/app/public/Market').'/'.$dm['logo'] ?? '',
                                    ...
                                    <td>
                                        <?php if(isset($dm->vendor->status)): ?>
                                            ...
                                            <label class="toggle-switch toggle-switch-sm" for="stocksCheckbox<?php echo e($dm->id); ?>">
                                                <input type="checkbox" data-url="<?php echo e(route('admin.Market.status',[$dm->id,$dm->status?0:1])); ?>" data-message="<?php echo e(translate('messages.you_want_to_change_this_market_status')); ?>" class="toggle-switch-input status_change_alert" id="stocksCheckbox<?php echo e($dm->id); ?>" <?php echo e($dm->status?'checked':''); ?>>
                                    ...
                                    <td>
                                        <div class="btn--container justify-content-center">
                                            <a class="btn btn-sm btn--primary btn-outline-primary action-btn"
                                                href="<?php echo e(route('admin.market.edit',[$dm['id']])); ?>" title="<?php echo e(translate('messages.edit_Market')); ?>"><i class="tio-edit"></i>
                                            </a>
                                            <a class="btn btn-sm btn--warning btn-outline-warning action-btn"
                                                href="<?php echo e(route('admin.market.view',[$dm['id']])); ?>" title="<?php echo e(translate('messages.view_Market')); ?>"><i class="tio-invisible"></i>
                                            </a>
                                        </div>
                                    </td>
                                </tr>
                        ...
                        <div class="page-area px-4 pb-3">
                            <div class="d-flex align-items-center justify-content-end">
                                <div>
                                    <?php echo $markets->appends(request()->all())->links(); ?>
...
<?php echo $__env->make('layouts.admin.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\wamp64\www\Mashe-Backend-main\resources\views/admin-views/market/list.blade.php ENDPATH**/ ?>