...
<body class="footer-offset">
    ...
    <!-- JS Front -->
    <script
        src="{{ dynamicAsset('public/assets/admin/vendor/hs-navbar-vertical-aside/hs-navbar-vertical-aside-mini-cache.js') }}">
    </script>
    ...
    <script>
        "use strict";
        var country_id = "{{auth('admin')->user()->country_id}}";
        if (country_id) {
            $.ajax({
        ...
        function route_alert(route, message, title = "{{ translate('messages.are_you_sure') }}", processing = false) {
            if (processing) {
                Swal.fire({
                    ...
                    input: 'text',
                    html: message + '<br/>' +
                        '<label>{{ translate('messages.Enter_Processing_time_in_minutes') }}</label>',
