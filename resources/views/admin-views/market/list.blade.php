...
@section('title',translate('market_List'))
...
@section('content')
    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <h1 class="page-header-title"><i class="tio-filter-list"></i> {{translate('messages.markets')}} <span class="badge badge-soft-dark ml-2" id="itemCount">{{$markets->total()}}</span></h1>
        </div>
        <!-- End Page Header -->
                ...
                <div class="col-sm-6 col-md-3">
                    <div class="select-item">
                        ...
                        <select name="market_model"
                                data-url="{{url()->full()}}" data-filter="market_model"
                        data-placeholder="{{translate('messages.all')}}" class="form-control js-select2-custom set-filter">
        ...
        <div class="row g-3 mb-3">
            <div class="col-xl-3 col-sm-6">
                <div class="resturant-card bg--1">
                    <h4 class="title" id="itemCount" >{{$markets->total()}}</h4>
                    <span class="subtitle">{{translate('messages.total_markets')}}</span>
            ...
            <div class="col-xl-3 col-sm-6">
                ...
                <div class="resturant-card bg--2">
                    ...
                    <h4 class="title">{{$active_markets}}</h4>
                    <span class="subtitle">{{translate('messages.active_markets')}}</span>
            ...
            <div class="col-xl-3 col-sm-6">
                <div class="resturant-card bg--3">
                    ...
                    <h4 class="title">{{$inactive_markets}}</h4>
                    <span class="subtitle">{{translate('messages.inactive_markets')}}</span>
            ...
            <div class="col-xl-3 col-sm-6">
                <div class="resturant-card bg--4">
                    ...
                    <span class="subtitle">{{translate('messages.newly_joined_markets')}}</span>
        ...
        <ul class="transaction--information text-uppercase">
            ...
            <li class="text--danger">
                ...
                <div>
                    ...
                    <span>{{translate('messages.total_market_withdraws')}}</span> <strong>{{\App\CentralLogics\Helpers::format_currency($market_withdraws)}}</strong>
                </div>
            </li>
        </ul>
        <!-- Transaction Information -->
                ...
                <div class="card">
                    ...
                    <div class="card-header py-2 border-0">
                        <div class="search--button-wrapper">
                            <h3 class="card-title">{{translate('messages.markets_list')}}</h3>
                            <form class="my-2 ml-auto mr-sm-2 mr-xl-4 ml-sm-auto flex-grow-1 flex-grow-sm-0">

                                <div class="input--group input-group input-group-merge input-group-flush">
                                    <input id="datatableSearch_" type="search" name="search" class="form-control"
                                          value="{{ request()?->search  ?? null }}"  placeholder="{{ translate('Ex:_search_by_market_name_of_Phone_number') }}" aria-label="{{translate('messages.search')}}" required>
                            ...
                            <div class="hs-unfold ml-3">
                                ...
                                <div id="usersExportDropdown"
                                    ...
                                    <a target="__blank" id="export-excel" class="dropdown-item" href="{{route('admin.market.markets-export', ['type'=>'excel',
                   request()->getQueryString()])}}">
                                    ...
                                    <a id="export-excel" class="dropdown-item" href="{{route('admin.market.markets-export', ['type'=>'csv',request()->getQueryString()])}}">
                                        <img class="avatar avatar-xss avatar-4by3 mr-2"
                    ...
                    <div class="table-responsive datatable-custom resturant-list-table">
                        <table id="columnSearchDatatable"
                            ...
                            <tr>
                                ...
                                <th class="initial-58">{{translate('messages.market_info')}}</th>
                            ...
                            <tbody id="set-rows">
                            @foreach($markets as $key=>$dm)
                                <tr>
                                    <td>{{$key+$markets->firstItem()}}</td>
                                    <td>
                                        <a href="{{route('admin.market.view', $dm->id)}}" alt="view Market" class="table-rest-info">
                                    ...
                                    <td>
                                        @if(isset($dm->vendor->status))
                                            ...
                                            <label class="toggle-switch toggle-switch-sm" for="stocksCheckbox{{$dm->id}}">
                                                <input type="checkbox" data-url="{{route('admin.Market.status',[$dm->id,$dm->status?0:1])}}" data-message="{{translate('messages.you_want_to_change_this_market_status')}}" class="toggle-switch-input status_change_alert" id="stocksCheckbox{{$dm->id}}" {{$dm->status?'checked':''}}>
                                    ...
                                    <td>
                                        <div class="btn--container justify-content-center">
                                            <a class="btn btn-sm btn--primary btn-outline-primary action-btn"
                                                href="{{route('admin.market.edit',[$dm['id']])}}" title="{{translate('messages.edit_Market')}}"><i class="tio-edit"></i>
                                            </a>
                                            <a class="btn btn-sm btn--warning btn-outline-warning action-btn"
                                                href="{{route('admin.market.view',[$dm['id']])}}" title="{{translate('messages.view_Market')}}"><i class="tio-invisible"></i>
                                            </a>
                                        </div>
                                    </td>
                                </tr>
                        ...
                        <div class="page-area px-4 pb-3">
                            <div class="d-flex align-items-center justify-content-end">
                                <div>
                                    {!! $markets->appends(request()->all())->links() !!}
                                </div>
                            </div>
                        </div>
                    </div>
