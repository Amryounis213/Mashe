...
class ZoneController extends Controller
{
    public function index(Request $request)
    {
        ...
        $country_id = auth('admin')->user()->country_id ;

        $zones = Zone::when(!is_null($country_id) , function($q) use($country_id){
            $q->where('country_id' , $country_id);
        })->
        withCount(['restaurants','deliverymen' , 'categories'])
    ...
    public function getZonesByCountry($country_id)
    {
        ...
        $admin->country_id = $country_id ;
        $admin->save();

        
        $zones = Zone::Active()->where('country_id', $country_id)->get();
